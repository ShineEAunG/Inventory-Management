...
public class CategoryService : ICategoryService
{
    ...
    public async Task<CategoryDetails?> GetById(Ulid categoryId)
    {
        ...
        var categoryDetails = new CategoryDetails
        {
            ...
            ModifiedAt = categoryInDb.ModifiedAt,
            ModifiedBy = categoryInDb.ModifiedBy
        };
        return categoryDetails;
    }

    public async Task<OperationResult> Update(UpdateCategoryDetails categoryDetails)
    {
        ...
        if(categoryDetails.ModifiedAt != categoryInDb.ModifiedAt)
            return new OperationResult(false, "Someone Updated already please refresh and try again");
        var userEmail = _httpContextAccessor.HttpContext?.User?.FindFirst("email")?.Value;
        userEmail = string.IsNullOrEmpty(userEmail) ? Creator.System : userEmail;
        ...
        categoryInDb.ModifiedAt = DateTimeOffset.UtcNow.ToString();
